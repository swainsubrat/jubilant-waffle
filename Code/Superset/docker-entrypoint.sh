...
# VERSION 1.0 (apache-superset version:0.29.rc5)

# DESCRIPTION: superset docker container entrypoint file
# Modified/Revamped version of the https://github.com/apache/incubator-superset/blob/master/contrib/docker/docker-init.sh
# Taken from https://github.com/abhioncbr/docker-superset/blob/master/script/docker-entrypoint.sh

# Modifications made: 
# 1) ln:54 - changed check for initializing superset such that superset is initialized whenever USER_COUNT=0
# 2) ln:62 - prevented superset from loading examples. This serves two purposes: a) prevents faulty initialization of superset b) user can see only his datasets while creating new charts
